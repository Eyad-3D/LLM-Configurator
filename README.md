...
## Hardware-calibrated speed estimates (v0.3.0)

Starting the questions launches an isolated synthetic calibration process alongside metadata retrieval. It reads no model files and downloads no weights. Calibration runs with a 12-second worker timeout; completed CPU measurements survive a GPU timeout. Results are cached locally for seven days and matched to the hardware/driver fingerprint. **Recalibrate speed** refreshes them, and `python -m llm_configurator calibrate` provides the same operation from the CLI.

CPU measurements use NumPy native operations: sustained read+write copy traffic over up to two 128 MiB buffers, followed by unpacking generated 4-bit values and matrix-vector operations, with up to eight worker threads (one BLAS thread each). Buffer sizes are reduced when memory is constrained. These are synthetic proxies, not STREAM results or llama.cpp kernels. Cache effects, other applications, power limits and thermal state affect readings.

On NVIDIA systems, the installed CUDA driver is used to measure device-memory copy throughput, pinned host-to-device and device-to-host transfers, and a small-transfer latency proxy. Up to four visible GPUs are probed and matched by UUID. There is no CUDA toolkit or PyTorch dependency. Unsupported drivers, limited free VRAM and non-NVIDIA GPUs remain explicitly uncalibrated; advertised bandwidth is never substituted silently. GPU inference computation is not benchmarked by this version.

The estimator covers the existing dense Qwen2/Qwen3/Llama catalogue with Q4_K_M, Q5_K_M, Q6_K and Q8_0. It uses model-file bytes, quantization size approximations, layer placement and FP16 KV-cache traffic at the selected context. CPU and GPU portions are combined with transfer overhead for split execution. The CPU lower bound uses both measured copy and synthetic quantized-matrix rates; its upper bound uses copy bandwidth because optimized fused inference kernels can greatly exceed the unpacking proxy. GPU estimates are bandwidth-only.

**Ranges are low-confidence heuristic envelopes, not validated confidence intervals or guarantees.** Version 1 assumes CPU bandwidth efficiency of 35–80% and GPU efficiency of 10–55% relative to copy probes, plus a 1–4x transfer-overhead envelope. Those sensitivity assumptions are explicit and still need calibration against real llama.cpp results. Runtime kernels, precision-specific arithmetic, attention implementation, cache reuse and scheduling can put actual speed outside the range. Prompt ingestion and time to first token are not estimated. Concurrent-session throughput is withheld because batching requires a separate model.

Cards show estimated generation tok/s, low confidence and a likely-meets/borderline/likely-below target assessment. **Speed priority** can use the lower estimate to order unmeasured configurations. Estimates do not reject configurations or satisfy the strict local-speed-test filter. Matching real model benchmarks always take precedence. The detail view and JSON export retain the measurement date, method and limitations.
