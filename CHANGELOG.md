...
- **Resumable, verified downloads**, including models split into several files. Pause/cancel keeps partial files for later.
...
- **Auto-tuning** of threads, batch sizes, flash attention, graphics-card layers, notepad (KV cache) compression and MoE expert placement within a 1–15 minute budget, skipping settings that would not fit in memory.
...
- **Opt-in community results**: anonymised sharing via a pre-filled GitHub issue you submit yourself, and import of shared results.
...
- **CI** on Windows, macOS and Linux, a packaging check, and a tag-only release workflow for PyPI.
