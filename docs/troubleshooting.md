...
- Linux: Vulkan if a graphics card is found, otherwise CPU; ARM (arm64) builds where llama.cpp publishes them.

`llm-config runtime status` shows what is installed. If the graphics card isn't used (the app says the GPU backend is unavailable), update your graphics driver. On Linux the Vulkan build needs Vulkan drivers (for example the `mesa-vulkan-drivers` package, or your vendor's driver).
...
The file is damaged or changed. Delete it (remove it in the app, or delete the file by hand) and download again.
