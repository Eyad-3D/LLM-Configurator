...
- Each step header shows its state: `Checking… / Ready / Not installed / Not downloaded / Found on your disk / Waiting (+reason) / Working… / Paused / Done / Needs attention / Demo / Server running`.
...
- **Test/tune/serve/quality** show "Waiting: get the engine first / download the model first" until steps 1–2 are done. In demo mode they show the demo message instead.
...
- **api**:
  ...
  6. Download progress: send `done`/`total` in bytes plus `bytes_per_second`, `eta_seconds` (as in §4.2). Runtime install progress: bytes too.
